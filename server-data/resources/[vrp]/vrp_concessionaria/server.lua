        ...
        if veiculo then
            ...
            if #getVeiculo == 0 then
                ...
                if parseInt(rows[1].qtd) >= veiculo.estoque then
                    ...
                    if isVendedor then valor = valor * 0.6 end

                    if vRP.tryFullPayment(user_id, valor) then
                        vRP.execute("vRP/add_vehicle", { user_id = user_id, vehicle = veiculo.model})
                        --vRP.execute("custom:u" .. user_id .. "veh_" .. veiculo.model, json.encode())
                        --vRP.execute("vRP/add_data",{dkey = "custom:u" .. user_id .. "veh_" .. veiculo.model})
                        TriggerClientEvent("vrp_concessionaria:notify", source,"Oba!", "Pagou <b>$" .. vRP.format(parseInt(valor)) .. "</b>.", "success")
                        return true
                    else
    ...
    if user_id and actived[parseInt(user_id)] == 0 or not actived[parseInt(user_id)] then
        ...
        if veiculo then
            ...
            if parseInt(price) > 0 then
                TriggerClientEvent("vrp_concessionaria:notify", source, "Oba!","Recebeu <b>$" .. vRP.format(parseInt(price)) .. "</b>.", "success")
            end
