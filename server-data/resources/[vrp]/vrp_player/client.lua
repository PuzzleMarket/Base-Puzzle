...
Citizen.CreateThread(function()
    while true do
		...
		if IsPedInAnyVehicle(ped) then
			idle = 5
			if IsControlPressed(1,246) and IsPedInAnyVehicle(ped, true) then -- Y		
				TriggerServerEvent('nitro')
				Citizen.Wait(100)
			end
		end
	Citizen.Wait(idle)
...
RegisterNetEvent('puzzle:nitro')
AddEventHandler('puzzle:nitro', function()
