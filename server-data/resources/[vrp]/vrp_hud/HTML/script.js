$(document).ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (data.fuel != null) {
            $('.fuel-container').css('background', `linear-gradient(to top, rgba(225, 51, 49, 0.5) ${data.fuel}%, rgba(0, 0, 0, 0.5) ${data.fuel}%)`);
            $('.fuel-container-bar').css('background', `linear-gradient(to right, rgba(225, 51, 49, 0.5) ${data.fuel}%, rgba(0, 0, 0, 0.5) ${data.fuel}%)`);
        }
        ...
        if (data.health != null) {
            $('#health').css('background', `linear-gradient(to top, rgba(225, 51, 49, 0.5) ${data.health}%, rgba(0, 0, 0, 0.5) ${data.health}%)`);
        }

        if (data.armour != null) {
            $('#armour').css('background', `linear-gradient(to top, rgba( 49, 116, 225, 0.5) ${data.armour}%, rgba(0, 0, 0, 0.5) ${data.armour}%)`);
        }
