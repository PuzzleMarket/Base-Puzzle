...
-- HUD
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("mapa",function(source,args)
	...
	if isOverlappingMenuActive == false then
		...
		if lastPauseMenuIsActive == false then
			...
			if pedVehicle == 0 then
				...
				local vehicleSpeed = math.ceil(GetEntitySpeed(pedVehicle) * 3.605936)
				...
				if lastVehicleSpeed == nil or lastVehicleSpeed ~= vehicleSpeed then
					...
					SendNUIMessage(
						{
							kmh = vehicleSpeed,
							maxSpeed = math.ceil(GetVehicleMaxSpeed(pedVehicle) * 3.605936)
						}
					)
				end
